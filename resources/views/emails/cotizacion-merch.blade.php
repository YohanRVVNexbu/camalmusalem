...
@section('subject', 'Cotización de merch')

@section('content')
    <h2>¡Hola, {{ $cotizacion->nombre }}!</h2>
    <p>Recibimos tu cotización del producto que te interesa. Un asesor de Toyota Musalem se pondrá en contacto contigo a la brevedad.</p>

    <div class="summary">
        <div class="summary-row">
            <span class="summary-label">Producto</span>
            <span class="summary-value">{{ $cotizacion->merch_nombre }}</span>
        </div>
        @if ($cotizacion->merch_precio)
        <div class="summary-row">
            <span class="summary-label">Precio referencial</span>
            <span class="summary-value">{{ $cotizacion->merch_precio }}</span>
        </div>
        @endif
        <div class="summary-row">
