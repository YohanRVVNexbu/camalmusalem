...
const branches = [
    ...
    },
    {
        ...
        address: 'Aristía #358',
        mapsUrl: 'https://www.google.com/maps/search/Aristia+358,+Ovalle,+Chile',
    },
];

export default function AccesorioShow({ accesorioId, footer }: { accesorioId: string; footer: any }) {
    ...
    const accesorio = {
        nombre: 'Hilux GR Lego Rasti',
        precio: '$ 59.000',
        descripcion: '¡Construye tu propia réplica del icónico Toyota Hilux con el set de construcción Rasti Lego! Con 359 piezas, ofrece una experiencia de construcción desafiante y gratificante, y una vez completado, podrás disfrutar de aventuras emocionantes con este impresionante modelo a escala. ¡Déjate llevar por la imaginación y la creatividad con el Toyota Hilux Rasti Lego!',
        images: [null, null, null, null], // placeholders
    };

    const prevImage = () => setCurrentImage((prev) => (prev === 0 ? accesorio.images.length - 1 : prev - 1));
    const nextImage = () => setCurrentImage((prev) => (prev === accesorio.images.length - 1 ? 0 : prev + 1));

    return (
        <>
            <Head title={`${accesorio.nombre} — Accesorios Toyota Musalem`} />
                        ...
                        <div className="flex w-[65%] flex-col gap-5">
                            ...
                            <div className="relative overflow-hidden rounded-[20px]">
                                <div className="flex h-120 w-full items-center justify-center bg-gray-100">
                                    <span className="text-base text-black/30">Imagen del accesorio</span>
                                </div>
                                <button
                                    onClick={prevImage}
                        ...
                        <div className="flex w-[35%] flex-col gap-5 rounded-[20px] bg-white p-7.5">
                            ...
                            <div className="flex flex-col gap-1.5">
                                ...
                                <div className="rounded-[10px] bg-[#EAEAF1] p-4">
                                    <span className="text-[28px] font-semibold uppercase leading-none text-black">{accesorio.precio}</span>
                                </div>
                            </div>
                                ...
                                {branches.map((branch) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-col gap-2">
                                            <span className="text-base leading-none text-black/60">{branch.name}</span>
                                            <span className="text-base leading-none text-black/60 underline">{branch.address}</span>
                                        </div>
                                    </a>
                                ))}
                            </div>
