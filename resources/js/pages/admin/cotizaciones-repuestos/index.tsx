...
export default function CotizacionesRepuestosIndex({ cotizaciones }: { cotizaciones: Cotizacion[] }) {
    const { flash } = usePage<{ flash: { success?: string } }>().props;

    const [filtro, setFiltro] = useState<SeguimientoFiltro>('activas');
    const visibles = filtrarSeguimiento(cotizaciones, filtro);

    const marcarLeido = (id: number) => router.patch(`/admin/cotizaciones-repuestos/${id}/leido`);
    const eliminar = (id: number) => {
        if (confirm('¿Eliminar esta cotización?')) {
            router.delete(`/admin/cotizaciones-repuestos/${id}`);
        }
    };
    ...
    return (
        <AdminLayout breadcrumbs={[{ title: 'Dashboard', href: '/admin' }, { title: 'Cotizaciones repuestos', href: '/admin/cotizaciones-repuestos' }]}>
            <Head title="Admin — Cotizaciones de repuestos" />
                            ...
                            <div key={c.id} className={`rounded-lg border p-5 transition-colors ${c.leido ? 'bg-background' : 'border-primary/30 bg-primary/5'}`}>
                                <div className="flex items-start justify-between gap-4">
                                    <div className="flex flex-1 flex-col gap-1 min-w-0">
                                        ...
                                        <p className="font-medium">{c.repuesto_nombre}{c.repuesto_precio ? ` — ${c.repuesto_precio}` : ''}</p>
                                ...
                                <SeguimientoControls tipo="cotizacion-repuesto" id={c.id} estado={c.estado} nota={c.nota_seguimiento} />
                            </div>
                        ))}
                    </div>
                )}
