export type PostventaWhatsappConfig = { phone?: string; message?: string } | null;
...
function buildHref(config: NonNullable<PostventaWhatsappConfig>, productName: string): string | null {
    const digits = (config.phone ?? '').replace(/\D/g, '');
    if (digits.length < 8) return null;
    const base = (config.message ?? '').trim() || 'Hola, quiero cotizar por el producto';
 ...
 * Si la sección está oculta o no hay teléfono válido, no renderiza nada.
 */
export function PostventaWhatsapp({ config, productName }: { config: PostventaWhatsappConfig; productName: string }) {
    if (!config) return null;
    const href = buildHref(config, productName);
    if (!href) return null;
